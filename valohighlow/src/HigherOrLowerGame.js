...
const HigherOrLowerGame = () => {
    ...
    useEffect(() => {
        ...
        const tNum = localStorage.getItem('tourney');
        if (tNum) {
            setTourney(tNum);
            startGame(tNum);
            console.log(jsonData[tNum].stats[0]['kills']);
        } else{
            startGame(0);
            console.log(jsonData[0].stats[0]['kills']);
        }
    ...
    const startGame = (tNum) => {
        ...
        console.log(jsonData[tNum].name);

        if (jsonData[tNum].name.includes("Americas")){
            setColor("#FF570C");
            setTextColor("#FFFFFF");
        } else if (jsonData[tNum].name.includes("Pacific")){
            setColor("#01D2D7");
            setTextColor("#0E1822");
        } else if (jsonData[tNum].name.includes("EMEA")){
            setColor("#D5FF1D");
            setTextColor("#0E1822");
        } else if (jsonData[tNum].name.includes("China")){
            setColor("#E73056");
            setTextColor("#FFFFFF");
        } else if (jsonData[tNum].name.includes("Masters")){
            setColor("#9464F6");
            setTextColor("#FFFFFF");
        } else if (jsonData[tNum].name.includes("LOCK")){
            setColor("#01A990");
            setTextColor("#FFFFFF");
        }else{
            setColor("#C6B275");
        ...
        recent.length = 0;
        const player = Math.floor(Math.random() * jsonData[tNum].stats.length);
        recent.push(player);
        setCurrentPlayer(player);
        setCurrentNumber(jsonData[tNum].stats[player][selectedStat]);
        generateNextNumber(selectedStat, player, tNum);
    };

    const generateNextNumber = (selectedStat, currentPlayer, tNum) => {
        console.log("-----------------");
        for (const element of recent) {
            console.log(jsonData[tNum].stats[element]["username"]);
          }

        let nextPlayer;
        do {
            nextPlayer = Math.floor(Math.random() * jsonData[tNum].stats.length);
        } while (currentPlayer === nextPlayer || recent.includes(nextPlayer) || jsonData[tNum].stats[nextPlayer][selectedStat]===null);
        
        if (recent.length > Math.floor(jsonData[tNum].stats.length*.1)){
            recent.shift();
        }
        recent.push(nextPlayer);
        setNextPlayer(nextPlayer);
        setNextNumber(jsonData[tNum].stats[nextPlayer][selectedStat]);
        console.log(jsonData[tNum].stats[nextPlayer][selectedStat])
    ...
    const handleGuess = (upScore) => {
        ...
        if ((upScore) % 5===0 && upScore !==0){
            ...
            } while (selectedStat === stat || (stat===2 && name === "Champions Tour 2024: China Kickoff"));
            setStat(selectedStat);
    ...
    const closeModal = (index) =>{
        console.log("TRNY: "+index);
        setModal(false);
        setTourney(index);
        localStorage.setItem('tourney', index);
        // setTourney(1);
        startGame(index);  
    };
    ...
    return (
        <div>
            ...
            <div className="split right" style={{backgroundColor: color, color: textColor}}>
            <div className="centered">
        ...
        <div id="myModal" className="modal" style={{display: modal ? 'block' : 'none'}}>
            ...
            <div className="modal-content" >
                ...
                {jsonData.map((trny, index) => (
                    <a href='#' key={index} className="trny-item hvr-shrink" onClick={() => closeModal(index)}>
